...
if (vary_values && !set_frames){
  printf("Number of frames is not set correctly");
  exit(1);
}

if (set_frames && !set_basename){
    ...
    if(op[i].opcode == VARY){
      ...
      for (current_frame = start_frame; current_frame <= end_frame; current_frame++){
        struct vary_node * new_knob = (struct vary_node*)malloc(sizeof(struct vary_node));
                ...
                new_knob -> value = start_value + (current_frame - start_frame) * delta;
                new_knob -> next = knobs[current_frame];
                knobs[current_frame] = new_knob;
      }
    }
  }
 return knobs;
